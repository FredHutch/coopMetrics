...
pullUncachedContributorData <- function(uncachedContributorPaths,
                                        owner = "FredHutch",
                                        repo = "coop") {
  # pull earlist commit date for unknown paths
  uncachedContributorDates <- uncachedContributorPaths %>%
    map_dbl(~ path2OldestCommitDate(owner = owner,
                                    repo = repo,
                                    path = .x)) %>%
    as_date()
  uncachedContributorData <- tibble(path = uncachedContributorPaths,
                                    commitDate = uncachedContributorDates,
                                    handle = path2Contributor(uncachedContributorPaths))
  return(uncachedContributorData)
}

calcContributorCommitData <- function(contributorData) {
  contributorDataCount <- knownContributorData %>%
    mutate(month = month(commitDate), year = year(commitDate))%>%
    count(month, year)
...
calcContributorStats <- function(contributorDateDf,
  ...
  if (length(uncachedContributorData) != 0) {
    uncachedContributorData <- pullUncachedContributorData(uncachedContributorPath, owner = owner, repo = repo)
  }
  contributorsByMonthYear <- bind_rows(knownContributorData, uncachedContributorData) %>%
  ...
  # newContributorNames <- as.character(pathToContributor(newContributorDf$path))
  #
